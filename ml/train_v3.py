"""
Train LegoGPT v3 — fast practical setup.

Sweet spot for CPU training:
- 8M params (vs 5M v1, 15M v2): d_model=320, n_layers=6, n_heads=8.
- Context 512 tokens (matches v1).
- Dataset: mid-range only (566 sequences, no sliding window).
- 30 epochs ~2 hours CPU.
- Cosine LR + warmup.
"""
...
class MPDDatasetV1(Dataset):
    """Use the v1 tokenized dataset (mid-range 30-150 pieces, 566 sequences)."""
...
def main():
    ...
    save_path = ml_dir / "model_v3.pt"
    log_path = ml_dir / "train_v3.log"
    log = []
