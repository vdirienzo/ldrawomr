#!/usr/bin/env python3
"""
Parser unificado cross-corpus 3.0: 532 sets legacy + 906 sets Plan D (5 cohorts).
Total: 1438 sets OMR.

New cohorts (Plan D):
- modern: 287 sets (official_modern themes)
- technic: 175 sets (Technic family)
- specialty: 165 sets (Brickheadz, Architecture, Modular Buildings, Ideas, etc.)
- licensed: 121 sets (Star Wars, Harry Potter, etc.)
- classic_gaps: 158 sets (filling official_classic themes not in earlier cohorts)

Genera cross_corpus3_stats.json + per_set_stats_<N>.json.
"""
import json
import re
import sys
...
def main():
    ...
    new_cohorts = ['modern', 'technic', 'specialty', 'licensed', 'classic_gaps']
    new_results = {}
        ...
        for meta in setlist:
            ...
            if not path.exists():
                ...
                if '-' in full:
                    ...
                    num, q = full, meta.get('qualifier', '1')
                base = f"{num}-{q}"
                candidates = list(Path(dir_path).glob(f"{base}*.mpd"))
                if candidates:
                    path = candidates[0]
                else:
                    print(f"WARN missing {cohort}: {fname}", file=sys.stderr)
                    continue
            try:
    ...
    # Global (532 + 906 = 1438 sets if all downloaded)
    ...
    Path('analysis/cross_corpus3_stats.json').write_text(json.dumps(out, indent=2, ensure_ascii=False))
