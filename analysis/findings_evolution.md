...
| parts/set (media) | 186.7 (min 0, max 2 834) |
...
| Década | Sets | % corpus | parts | % parts | parts / set (media) |
|---|---|---|---|---|---|
| 80s | 25 | 25 % | 3 090 | 16.6 % | 123.6 |
| 90s | 75 | 75 % | 15 580 | 83.4 % | 207.7 |

El corpus está sesgado hacia los 90s (3× más sets, 5× más parts), lo que afecta cualquier comparativa de frecuencias absolutas — uso **medias, medianas y proporciones** para comparar décadas.
...
|---|---|---|---|
| Sets | 25 | 75 | +200 % |
| parts totales | 3 090 | 15 580 | +404 % |
| **parts/set (media)** | **123.6** | **207.7** | **+68 %** |
| parts/set (mediana) | 101 | 81 | −20 % |
| parts/set (Q1 / Q3) | 36 / 123 | 41 / 234 | rango más amplio en 90s |
| parts/set (max) | 379 (Train 7730-1) | 2 834 (Pirates 6286-1) | 7.5× |
...
| % parts Technic-flagged (8 parts clave) | 13.4 % | 19.4 % | +45 % |
...
## 3. Vocabulario de parts: 80s vs 90s

Top 15 parts agregadas por década (suma de frecuencias en los top-30 de cada set):
...
- **parts Technic con agujero para eje** (`754`, `6141`, `3710`, `3820`, `3660`) están presentes en AMBAS décadas, pero su proporción crece 13.4 % → 19.4 % del total de parts.
- **parts nuevas en 90s que no aparecen en el top-15 de 80s**: `3022` (plate 2×2), `2420` (corner plate 2×2), `2412b` (tile 1×2). Las **tiles planas** (`2412b`, 157 apariciones) son un marcador de los 90s — son muy usadas en Town/Castle para acabados lisos.
...
- **parts populares en 80s que ya no en 90s**: `3665a` (slope brick) baja de 55 a (no aparece en top-15 90s); `4070` (brick round 1×1) baja de 45 a ausente.
...
**Lectura**:
- Las custom parts se concentran en **vehículos grandes y sets temáticos licenciados/non-System**: el Schooner Pirates (1993) es un outlier extremo con 21 custom parts — probablemente rats, pirate hats, cofres, anclas, cañones y similares. La escala del set (2 834 parts) absorbe el coste de diseñar molds únicos.
...
> **Conclusión parcial**: la hipótesis "los 90s introdujeron más parts licenciadas" **se cumple en volumen** (68 vs 10) **pero no en proporción** (16 % vs 20 % de sets). El corpus 80s está dominado por sets pequeños donde cualquier mold único infla la proporción.
...
| Año | Sets con BFC CERTIFY | themes |
|---|---|---|
| 1980 | 1 | Fabuland |
| 1982 | 2 | Boat, Town |
| 1984 | 1 | Castle |
...
> Nota: BFC CERTIFY es un **metadato del models digital**, no del set físico. Su distribución refleja prácticas de la comunidad OMR, no convenciones LEGO de fábrica.
...
| Set | Año | theme | Sub-builds | parts |
|---|---|---|---|---|
...
| Año | Sets | parts totales | Media parts | Media sub-builds | BFC | Custom |
|---|---|---|---|---|---|---|
...
- **1980 — Fabuland + Town**: el corpus tiene Fabuland 3602-1 (10 parts, BFC CERTIFY), Town 1591-1 (Danone Truck, 40 parts), Space 6821-1, Train 7730-1 (379 parts) y 7834-1. Town aparece clasificado como "Classic Town" en `theme_full`. **Fabuland** se retira del corpus tras 1985.
- **1984 — Castle Revival**: 3 Castle sets en 1984 (Supply Wagon 40 parts, Siege Tower 341 parts con BFC CERTIFY + 1 custom part). El corpus no incluye ningún Castle 1985–1986; regresan en 1986 (Armor Shop) y 1987 (Battering Ram).
- **1986 — Model Team debuta en el corpus**: 5510-1 Off-Road 4x4 (283 parts, 10 sub-builds, BFC CERTIFY, 4 custom parts). Model Team seguirá apareciendo hasta 1999.
- **1989 — Pirates launch**: el corpus solo captura **6235-1 Buried Treasure** (31 parts, 3 sub-builds, sin BFC, sin custom parts) — una muestra diminuta del lanzamiento Pirates. **El Black Seas Barracuda (6285-1) NO está en este corpus** (es la parts fundacional del theme).
...
- **1993 — Skull's Eye Schooner outlier**: 6286-1 Pirates con **2 834 parts, 33 sub-builds, 21 custom parts, BFC CERTIFY**. Es el set más grande del corpus y absorbe el grueso de las custom parts de los 90s. Model Team 5521-1 Sea Jet también en 1993 (391 parts).
...
- **1995 — Pirates + Train**: 6279-1 Skull Island (383 parts, BFC CERTIFY, 2 custom parts). 5 sets totales.
- **1996 — pico de variedad y de Model Team**: 14 sets (máximo del corpus). 5571-1 Giant Truck (Model Team, 1 703 parts, **40 sub-builds** — récord absoluto). Western 6765-1 (382 parts, 10 custom parts, BFC CERTIFY). Universal Building Set aparece (4 sets pequeños: South African Flag y 3 Danone Promotional).
- **1997 — año "de transición"**: 4 sets pequeños (Indian Kayak 23 parts, Speedboat 28, Outback Racer 54, Tribal Chief 19). Media de parts 31 — el año más bajo del corpus. Esto coincide con la crisis LEGO de finales de los 90.
- **1998 — Model Team pesado**: 5542-1 Black Thunder (485 parts, 28 sub-builds, 10 custom parts, BFC CERTIFY).
- **1999 — Star Wars peak + Model Team**: los 6 sets Star Wars del corpus son todos de 1999 (Lightsaber Duel 64 parts hasta TIE Fighter & Y-wing 360 parts). Model Team 5533-1 Red Fury (353 parts, 32 sub-builds, BFC CERTIFY). **1999 tiene la media de sub-builds más alta (14.9)**.
...
**Sí, en promedio (123.6 → 207.7 parts/set), pero NO en el set típico (mediana 101 → 81)**. La respuesta honesta es: el corpus tiene **más sets pequeños** en los 90s (Universal Building Set, Star Wars minifig-scale, Town promotional) Y **más sets gigantes** (Pirates 6286 con 2 834, Model Team 5571 con 1 703, Star Wars 7140/7150 con 328–360). La dispersión crece; la mediana baja.

### ¿Mayor modularidad (sub-builds) en los 90s?

**Sí, claramente.** Media 5.68 → 9.29 (+63 %). El corpus 90s tiene 5 de los 6 sets con más sub-builds, todos Model Team o Pirates/Star Wars. La práctica de **diseñar sets grandes como ensamblaje de sub-models** (cabina + chassis + carrocerías + detalles) se generaliza a finales de los 90.

### ¿Aparición de parts Technic en sets no-Technic?
...
| `4-4cyli.dat` / parts | 3.4 % | 11.5 % |
| % parts Technic-flagged (8 clave) | 13.4 % | 19.4 % |
...
2b. **parts Technic con agujero de eje (`6141`, `3710`, `3820`, `754`) se generalizan en 90s**: en 80s aparecen sobre todo en Castle/Train/Town grandes (6061, 6062, 7715, 6657). En 90s se diversifican a Model Team, Pirates, Star Wars, Western y Town vehículos. La lista de "sets con ≥30 parts Technic-flagged" pasa de 5 sets en 80s (1984–1987) a 18 sets en 90s.
...
4. **parts top-15 en 80s que salen del top-15 en 90s**: `3665a` (slope brick), `3660` (plate 1×2 Technic hole), `3666` (plate 1×4), `3009` (brick 1×6), `3063b` (round brick 2×2), `3795` (plate 2×6). Los 80s dependían más de plates anchos y slopes; los 90s diversifican hacia tiles planas, plates 2×N y Technic con eje.

5. **Custom parts correlacionan con tamaño, no con licencia**: los 6 Star Wars de 1999 tienen **0–1 custom parts cada uno**. El set con más custom parts (6286 Skull's Eye Schooner, 21) es **Pirates** — un theme propio de LEGO, no licenciado. La licencia SW en este corpus no introduce más molds únicos que Pirates o Model Team.
...
7. **STEPs reflejan sub-builds, no escala pura**: 90s duplican la media de STEPs (11 → 23) sin duplicar la media de parts (123 → 208). Esto es porque cada sub-build genera varios STEPs independientes — los 90s son **más densos en instrucciones** (más sub-models a documentar) incluso cuando el set no es más grande.
...
- El corpus omite **el grueso de Technic set** como theme separado — solo hay Universal Building Set (4 sets) y algunos "Model Team" cuentan con parts Technic. No podemos comparar System vs Technic.
...
- La **gran transformación técnica** es la adopción masiva de parts Technic (ejes, cilindros) dentro de sets System → permitido por Model Team y explotado por Star Wars.
...
- "Pirates end en 1992" — **falso**: hay Pirates 1993 (6286 Skull's Eye Schooner, 2 834 parts) y 1995 (6279 Skull Island).
...
- "Castle Revival 1984–1985" — sí (Siege Tower 6061, 341 parts, BFC CERTIFY).
- "1999: Star Wars peak" — sí (6 sets, hasta 360 parts y 22 sub-builds en X-wing).