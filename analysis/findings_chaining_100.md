# Análisis cross-corpus del chaining de parts — 100 sets OMR (1980-1999)
...
> **Volumen:** **18 670 parts · 839 sub-builds · 1 985 pasos · 112 parts embebidas (78 custom + 34 subparts)**
> **Periodo:** 25 sets ochenteros (3 090 parts) + 75 sets noventeros (15 580 parts)
...
| 2 | **3023.dat** | 590 | 3.16 % | **Plate 1×2.** parts estructural universal: relleno de chasis (Town), paneles (Space), contrapisos (Model Team), paredes internas (Castle). Top en Town (120), Model Team (157), Space (120), Train (76), Pirates (49), Boat (6), Promotional (10). |
...
| 5 | **3024.dat** | 368 | 1.97 % | **Plate 1×1.** Relleno de huecos. Top en Town (122), Model Team (102), Castle (46), Boat (no top), Sports (11). Universal: encaje entre parts grandes. |
...
| 9 | **754.dat** | 265 | 1.42 % | **Technic plate 1×2 con hole (lado corto).** Distribución bimodal: **Pirates 6286-1 = 173/265 = 65.3 %** (sujección de cañones y elementos giratorios del barco); Town 6657-1 = 92/265 = 34.7 %. Sólo 2 sets contienen esta parts en cantidad. |
...
| 15 | **3020.dat** | 162 | 0.87 % | **Plate 2×4.** parts grande. Top en Town. **Dato anómalo: 0 self-bigrams** pese a 162 parts — nunca se coloca adyacente a sí misma; siempre se usa como parts-puente entre otras. |
...
1. **Plates estándar (3023, 3004, 3024, 3623, 3022, 3020, 3021, 3460, 3622, 3666, 2431, 2412b): ~3 100 parts = 16.6 %** — la espina dorsal estructural de cualquier set OMR.
2. **Technic plates embebidas (3710, 3820, 754, 6141, 4070, 2412b, 4286, 3819, 3818): ~1 770 parts = 9.5 %** — la **conclusión clave**: incluso excluyendo el theme Technic, el 9.5 % de las parts son Technic plates con hole. Trains (5 sets) + Model Team (5 sets) son los grandes consumidores.
3. **Primitivas (4-4cyli): 1 896 parts = 10.2 %** — sesgo extremo dominado por **1 solo set** (6286-1 Pirates).

**theme dominante del top-30:** Town consume los plates estándar (3024=122, 3023=120, 6141=112, 754=92, 3710=80). Model Team consume 6141=136, 3710=119, 3623=89 (carrocerías curvas con Technic embebido). Pirates consume 4-4cyli=1792 + 754=173 (barcos con cañones).

---

## 2. Top 15 deltas posicionales entre parts consecutivas

Total agregado de los 15 deltas más frecuentes = **3 333 ocurrencias** sobre los **17 831 transitions totales** (18 670 parts − 839 sub-builds) = **18.7 % de las transiciones concentradas en 15 patrones**.

| # | Δ (dx, dy, dz) | Count | Tipo | Lectura |
|--:|-----------------|------:|------|---------|
| 1 | **(0, 0, 0)** | **541** | **identidad** | **Cruce de sub-builds:** parts N termina un sub-build en posición P, parts N+1 abre el siguiente sub-build en **la misma posición P**. Las 839 sub-builds producen 838 boundaries internos — pero este delta cuenta pares consecutivos, no boundaries: ~0.65 (0,0,0) por boundary. Indica que los autores OMR **reinician los sub-builds desde el origen** (0,0,0) — la primera parts de cada sub-build está en (0,0,0) y, si la última del anterior también estaba ahí, se produce este delta. Es **un artefacto del flujo de parsing**, no una colocación física. |
...
| 5 | (0, -8, 0) | 204 | estándar | **+1 plate vertical**: encadenar una plate sobre otra plate = 8 LDU. **El chaining vertical más común del corpus** (más frecuente en Y). |
...
**Conclusión:** los 4 deltas irregulares son **Technic-pin-driven**, no "parts inclinadas" como podría pensarse. Provienen específicamente de las **primitivas cilíndricas y Technic connectors** dentro de partes compuestas (4-4cyli embebida en cañones Pirates).

### 2.2 Lectura detallada de los deltas clave

- **(0,0,0) = 541** — la primera parts de cada sub-build es típicamente (0,0,0). Como cada sub-build arranca desde origen y los 4-4cyli de Pirates 6286-1 están todos cerca del origen de su sub-build, los cruces producen identidad.
- **(0,-8,0) = 204** — el chaining vertical más frecuente. Apilar plate sobre plate (8 LDU). Equivale a "rellenar vertical": el autor coloca 1 plate, luego rellena encima con otra plate del mismo tamaño.
- **(0,-24,0) = 131** — transición plate→brick: cuando dy=-24 aparece, es que el models **crece verticalmente** con un brick, no con otra plate. Es la "primera hilada de muro".
- **±100 / ±140 / ±180 / ±220 en X o Z** — **anchos no canónicos de bricks**: brick 1×5 (3004/3005-style) genera exactamente 100 LDU; brick 1×7 (3003) genera 140 LDU; brick 1×8 (3008) y brick 1×9 (3009) generan 160-180; parts de 11 studs generan 220. **Los muros largos se encadenan extremo-con-extremo en X o Z** usando estos deltas.

---

## 3. Top 20 bigramas (pares parts N → parts N+1)

Total bigramas en top-50 = **5 355 ocurrencias**: **4 871 self-bigrams (90.9 %)** + **484 cross-bigrams (9.1 %)**. El **chaining dominante es same-piece** (se rellena con la misma parts consecutivo).

| # | Bigrama | Count | Tipo | Lectura |
|--:|----------|------:|------|---------|
| 1 | **4-4cyli → 4-4cyli** | **1892** | primitiva auto | **1892 cilindros consecutivos** = **99.8 % de consecutividad** (1892/(1896-1)). Prácticamente todos los 4-4cyli del corpus están apilados uno tras otro en el flujo de parsing. Confirma que las primitivas se insertan **en bloque** dentro de un sub-build (el files .mpd lista los cañones uno a uno sin intercalar otra parts). |
...
- **Bigramas de primitivas auto-repetidos (4-4cyli → 4-4cyli, 754 → 754):** 2 patrones = **2 145 ocurrencias = 40.1 % del top-50**. **Bloques puros de la misma parts** (los files OMR escriben las primitivas en racimo).
...
## 4. Top 15 Y-layers (alturas de las parts)

| # | Y (LDU) | Count | % del top-15 | Lectura |
|--:|---------|------:|-------------:|---------|
| 1 | **0** | 991 | 13.4 % | **parts a nivel de suelo** (base de cualquier models). 991 parts = 5.3 % del corpus están exactamente en Y=0. |
...
| 13 | **+8** | 266 | 3.6 % | **parts sobre plano de referencia** (raro: signo + indica debajo del origen, lo que ocurre cuando el models se construye **hacia abajo** desde un plano superior). |
| 14 | **-88** | 230 | 3.1 % | **Plate + brick + brick + plate** = -8-24-24-8 = -64... no. Composición mixta. |
| 15 | **-128** | 214 | 2.9 % | **4 bricks** sobre plate base. |

**Total top-15 = 7 370 parts = 39.5 % del corpus**集中在 15 alturas canónicas (todas múltiplos de 8 LDU). El resto del corpus (60.5 %) se reparte en otras alturas.

### 4.1 Y-layers irregulares (no múltiplos de 8)

**Los top-15 Y son todos canónicos** (múltiplos de 8). Pero existen **195 valores Y irregulares distintos** sumando **2 148 parts** = **11.5 % del corpus** que está en alturas no estándar:
...
| -20 | 54 | Brick Technic |
| -14 | 50 | Composition |
| **-46.1** | **50** | **Slope 33° / 45° parts inclinadas** — esta signature aparece en muchos sets (Town, Castle, Model Team) |

**Lectura:** Las **Y irregulares son Technic-driven** (ruedas, slopes Technic, plates con offsets). Las parts inclinadas (slopes 33°, 45°) tienen origen descentrado, lo que genera Y no múltiplos de 8. Por eso el **Y = -46.1 (50 parts)** es signature de slopes: la base del slope 1×2 33° está a -46.1 LDU (compuesto: -24 brick + 8 plate + ~-22 pendiente). Aparece en 6511, 6514, 6348, 6525, 6428, 1772, 6666, 6331, 3314, 6550 (todos Town/Castle/Model Team).

**Conclusión:** los Y irregulares son **parts Technic embebidas + slopes**, no errores. El **88.5 % de las parts están en Y canónico** (múltiplos de 8).
...
### R1. **El 21.5 % de las parts tienen una matriz única en su set**
Suma de `unique_matrices` = **4 016** sobre **18 670** parts = **21.5 %**. Implicación: **78.5 % de las parts comparten matriz con al menos otra parts del mismo set**. La **matriz identidad domina** — la mayoría de colocaciones no requieren rotación ni reflexión.
...
### R3. **Solo 0.27 % de las parts tienen det<0 (reflejos)**
...
### R4. **112 parts embebidas = 0.6 % del corpus (78 custom + 34 subparts)**
- **Custom parts (parts licenciadas/estampadas propias): 78** — 0.42 % del corpus.
- **Subparts (sub-files .dat referenciados): 34** — 0.18 %.
- **Hires primitives: 0.**
- Distribución por theme:
  ...
  - **Train: 8 custom (0.3 %)** — 4558-1 aporta 8 (pero tiene 0 parts — son todas custom!).
  - **Castle: 5 custom (0.3 %).**
  - **Boat, Fabuland, Space, Star Wars: 1-2 custom cada uno.**

**Lectura:** las custom parts son **raras pero concentradas en sets estrella** (Black Seas Barracuda, Wild West sets, Model Team). **Sirven para parts con decoración impresa** (mástiles, velas, logotipos).
...
4 871 / 5 355 = **90.9 %**. Las parts se encadenan mayoritariamente **consigo mismas** (relleno en hilera), no entre tipos. **Excepción:** las Technic plates embebidas (3818/3819/3820, 6014/6015, 4624/3641) sí forman bigramas heterogéneos significativos — **porque las variantes de Technic plates son funcionalmente intercambiables** y se mezclan.

### R7. **El (0,0,0) es el delta más frecuente pero NOPlacement física**
541 ocurrencias = **3.0 % de todas las transiciones** (17 831 = 18 670 parts − 839 sub-builds). Es un **artefacto del parsing**: cuando se concatena el último parts de un sub-build con la primera del siguiente, **si ambos están en (0,0,0) (origen del sub-build)** aparece este delta. En LDraw, los sub-builds OMR típicamente arrancan desde el origen local, lo que produce la identidad.

### R8. **Model Team y Train son los themes "Technic embebido"**
- **Model Team (3 215 parts):** top parts son 3023 (157), **6141 (136)**, **3710 (119)**, 3024 (102), 3623 (89). Las Technic plates 6141/3710 son las #2 y #3.
- **Train (2 592 parts):** top son 3004 (105), 3023 (76), **3710 (29)**, **3063b (48)**, **3665a (46)**. Technic plates 3710/3063b/3665a en el top.
- **Town (3 553 parts):** también usa Technic plates (754=92, 3710=80, 6141=112).
- **Stars Wars (1 120 parts):** top son 756 (48, Technic liftarm), 3820 (28), 3003 (24), 2877 (24).
...
### R9. **El 39.5 % de las parts están en sólo 15 alturas canónicas**
7 370 / 18 670 = **39.5 %**. La concentración vertical es fuerte: cualquier models OMR se construye básicamente en **niveles de plate (-8 LDU) y brick (-24 LDU)** alternados.

### R10. **El corpus está sesgado hacia los 90s**
- 25 sets ochenteros = **3 090 parts = 16.6 %**.
- 75 sets noventeros = **15 580 parts = 83.4 %**.
...
**Implicación:** sets grandes y complejos (Model Team, Space, Star Wars, Train, Pirates) tienen **más sub-builds** — los autores OMR dividen el models en partes para hacerlo manejable. Town (32 sets pequeños) tiene menos sub-builds por set.
...
|----------|------:|
| Sets | 100 |
| parts totales | 18 670 |
| Sub-builds totales | 839 |
| Pasos totales | 1 985 |
| parts/paso promedio | 9.4 |
...
| Top parts | 4-4cyli.dat (1896, 94.5 % de 6286-1 Pirates) |
| Top delta | (0,0,0) = 541 (12.8 % de top-30) |
| Top Y-layer | Y=0 = 991 parts (5.3 %) |
| Top bigrama | 4-4cyli → 4-4cyli = 1892 (99.8 % consecutividad) |
| parts con 0 self-bigrams (anómala) | 3020.dat (plate 2×4, 162 parts) |

## Apéndice B — Distribución por theme

| Theme | Sets | parts | % corpus | Custom | Matrix-uniqueness |
|-------|-----:|-------:|---------:|-------:|------------------:|
...
**Lectura del apéndice:** **Pirates es el outlier en matrix uniqueness (64.0 %)** porque 6286-1 Black Seas Barracuda tiene 33 sub-builds con 1792 4-4cyli cada uno en posición distinta. Sin ese set, la media de matrix uniqueness del corpus caería a ~13 %. **Model Team es lo opuesto** (3.5 %) porque los sets MT tienen grandes superficies con la misma parts en la misma posición (carrocerías de coches).

---

*Análisis generado a partir de `cross_corpus_stats.json` y `per_set_stats.json` — todos los conteos citados son directamente de los files, sin redondeos.*
