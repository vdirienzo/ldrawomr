# Análisis profundo del ENCADENAMIENTO de piezas — 10252 VW Beetle + 10218 Pet Shop

> Datos crudos: `analysis/deep_stats.json` (6 662 líneas)
> Modelos fuente: `corpus/10252-1.mpd` (Beetle), `corpus/10218-1.mpd` (Pet Shop)
...
Los deltas son **diferencias de posición entre piezas consecutivas** (línea 1 tipo 1 → línea 1 tipo 1 siguiente) en el archivo, **NO** distancias absolutas entre piezas físicamente adyacentes. Reflejan el orden de inserción en el STEP.

### 1.1 Conteo combinado de los dos sets (lo más estable entre ambos modelos)

| Δ (dx, dy, dz)   | Significado físico                                                                  | 10252 | 10218 |
|------------------|-------------------------------------------------------------------------------------|------:|------:|
| (0, 0, 0)        | Misma posición exacta — pieza colocada en hueco abierto encima de la última          |    —  |   26  |
...
- **`(0, -8, 0) = 38+65 = 103 ocurrencias`** — El encadenamiento vertical más frecuente del corpus. Sumar exactamente 1 plate al Y de la pieza anterior: la pieza siguiente encaja en los studs superiores de la anterior (plate 8 LDU de alto + 0 sobre base).
- **`(0, 0, -40) = 22+68 = 90 ocurrencias`** — Reemplaza la pieza anterior en el mismo stud-set, 2 studs atrás. Indica que se está **rellenando una fila hacia el fondo del modelo**: brick 1×2 orientado en Z, o tile 1×2 en Z, o dos plates 1×2 seguidos en fila Z.
- **`(0, 0, -100) = 63 ocurrencias (sólo Pet Shop)**` — Distancia exacta de 5 studs en Z: coincide con la dimensión de un **brick 1×5 (3004 variante) o tile 1×5**. Aparece casi exclusivamente en Pet Shop porque el modelo de 3 plantas tiene paredes largas en Z.
...
- **`(100, 0, 0) = 36 ocurrencias (sólo Pet Shop)**` — Distancia **no múltiplo de 20**: no es encaje de studs. Corresponde a posiciones donde se coloca una pieza cuyo origen **NO** está en el centro de stud, sino descentrado (p.ej. puertas, ventanas, piezas Technic). Indica elementos de ancho 5 studs con tolerance (±10 LDU).

### 1.3 Deltas del mismo tipo (same-piece deltas)

Para entender cómo se **encadenan piezas idénticas** entre sí:

- **`30244.dat` (plate 1×2 con 1×1 cutout)** se repite con `(±40, 0, 0) = 4+4 = 8`: es una pieza que se usa para ir rellenando una hilera de **2 studs por pieza** consecutivamente (Brick Beetle tiene filas de plates con ventanas de 1×1).
...
- **`3024.dat` (plate 1×1)** en Pet Shop con **`(0, -8, 0) = 6` y `(0, 0, -40) = 7`**: la plate 1×1 se usa tanto para **apilar vertical** como para **saltar 2 studs en Z** (se usa como tapado de stud suelto entre piezas más grandes).
- **`3023.dat` (plate 1×2)** en ambos sets: `(0, 0, -40) = 3+5` (en Z) y `(0, 0, 40)=1` (otra orientación): se encadena en la **dirección larga de la pieza**.
...
Bigrama = pieza N → pieza N+1 en el orden del archivo.
...
**Lectura global del Beetle:** Predominio absoluto de plates (3023, 3024, 3794b, 3623) y tiles (3069b, 3070b, 85984) — modelo **plano y liso** con muchas superficies. Los bigramas misma-pieza = **se están construyendo hileras largas** (chasis, suelo, techo curvo).
...
La mayoría de los trigramas son runs de la misma pieza (Brick 1×1 tres veces = columna). Los **verdaderamente informativos** son los **heterogéneos**:
...
### 3.3 Trigramas de stacking vertical (todos los modelos)

Casi todos los trigramas AAA con `count >= 6` son **stacking vertical de plates** (`plate → plate → plate` con Δy=-8) o **stacking de la misma pieza con Δy=-24** (brick sobre brick).

Ejemplo Beetle: `3794b → 3794b → 3794b (count=20)` → "tres plates con groove consecutivas", generalmente apiladas en Y.

Ejemplo Pet Shop: `4216 → 4216 → 4216 (count=28)` → **28 trigramas de 3 liftarms apilados verticalmente** — la estructura interna del ático del Pet Shop es básicamente **un muro de Technic liftarms apilados**.
...
## 4. Rotaciones (matrices) por pieza
...
### 4.1 Top-10 piezas del Beetle con su rotación dominante

| Pieza    | count | Rotación dominante (matriz)                              | count | Por qué                                                                                   |
|----------|------:|----------------------------------------------------------|------:|-------------------------------------------------------------------------------------------|
...
|          |      | `-1 0 0 0 0 1 0 1 0` (espejo, det=-1)                    |     2 | **Raro**: aparece el flip solo en piezas donde se quiere romper simetría.                  |
...
### 4.2 Top-10 piezas del Pet Shop con su rotación dominante

| Pieza    | count | Rotación dominante                                       | count | Por qué                                                                                  |
|----------|------:|----------------------------------------------------------|------:|------------------------------------------------------------------------------------------|
...
1. **La matriz identidad siempre está en el top-3**, pero no siempre es la #1. En piezas **cuadradas o con simetría rotacional** (3024, 3005, 3070b, 6141) las 4 rotaciones cardinales se reparten el uso.
...
3. **Los flips (det=-1) son raros**: ~2-5% del total. Aparecen cuando:
   ...
   - se monta una pieza Technic en orientación invertida
   - hay una pieza con un detalle que sólo tiene sentido en una dirección (logos, pegatinas)
...
## 5. Reglas inferidas de encadenamiento

Cada regla incluye: observación numérica + interpretación física + ejemplo concreto del JSON.
...
- **Interpretación:** Cada vez que la siguiente pieza es otra plate (8 LDU de alto) y debe encajar en los studs superiores de la anterior, se resta exactamente 8 al Y. El origen de la pieza plate es su **base de studs** (no la cara superior), así que colocar una plate encima de otra plate requiere Y → Y - 8.
- **Ejemplo:** Trigrama `3023 → 3023 → 3023 (count=5 en Beetle)` con Δy=-8 cada vez → **una columna de 3 plates = 24 LDU vertical** (equivalente en altura a un brick, pero dividido en 3 capas).
...
- **Interpretación:** Cuando la pieza anterior es una plate (8 LDU) y la siguiente es un brick (24 LDU), la diferencia de altura es **24** (alto del brick), NO la suma 8+24=32. Esto es porque el origen del brick es **su base**, igual que la plate: el brick se coloca de modo que su base apoye en los studs superiores de la plate → Y = Y_plate - 24.
- **Ejemplo:** En Pet Shop, trigrama `3005 → 3005 → 3005 (count=12)` con Δy=-24 cada vez → **3 bricks 1×1 apilados = 72 LDU de columna vertical**.

### Regla 3 — Las piezas Technic liftarm (4216) se apilan verticalmente como si fueran bricks
...
- **Ejemplo:** Trigrama `4216 → 4216 → 4216 (count=28)` → **el ático del Pet Shop es literalmente un muro de 28 liftarms apilados verticalmente**.

### Regla 4 — Encadenar la misma pieza en X sin solape requiere Δx = 20 × ancho
...
- **Interpretación:** Para encadenar N piezas idénticas extremo con extremo en X sin solape, Δx = 20 × studs_de_la_pieza. El origen de la pieza está en el centro del stud-set, por eso 6 studs = 120 LDU entre centros.
- **Ejemplo:** Construcción de muro de 8 studs en Pet Shop = `3009 (1×6) + 3010 (1×4) extremo con extremo` — exactamente los bigramas heterogéneos `3009→3010 (count=24)` y `3010→3009 (count=22)`.
...
- **Interpretación:** En esquinas, las piezas no se centran en stud — se desplazan al corner. (20, 0, -20) = diagonal 1-stud (esquina de plate 2×2 o de 1×1 individual). (30, 0, -50) = esquina con offset raro: 30 LDU = 1.5 studs, 50 LDU = 2.5 studs — corresponde a colocar una pieza descentrada sobre la esquina de una base 4×4 con un 1×1 cortado en la esquina.
- **Ejemplo:** `3010.dat same_piece_delta (30, 0, -50) = 5 ocurrencias` → un brick 1×4 colocado en una esquina de base con desplazamiento de 1.5+2.5 studs (típico cuando el muro arranca en un corner cut).

### Regla 6 — Las piezas Technic pin (2780, 87087) se colocan cada 40-60 LDU (no 20)
...
- **Ejemplo:** Cuatro pins Technic 87087 colocados en fila a `(20, 0, ±1.4)` forman una **hilera Technic** con 1 stud de pitch (20 LDU) — esto es propio de engranajes/ejes del Beetle.
...
- **Ejemplo:** Secuencia de tiles del techo en Beetle: cada tile está girada 4° respecto a la anterior, creando la **silueta curvada del VW Beetle original**.
...
- **Ejemplo:** Buhardillas del ático del Pet Shop: 4 platos redondos 6141 orientados a 45° forman el detalle de las **ventanas circulares de las buhardillas**.

### Regla 9 — Los deltas no múltiplos de 20 identifican piezas con tolerancia o descentradas
...
- **Observación:** Deltas "raros" del corpus:
  ...
  - `(40.1, -57.3, 50)` en `99206.dat` — pieza con posición off-center.
  - `(-0.819 -0.574 0 ...)` en `3024.dat` y `2420.dat` — rotaciones a 35° (slopes).
- **Interpretación:** Cualquier delta en X o Z que **no sea múltiplo de 20 LDU** indica una pieza cuyo origen NO está en el centro del stud-set, o una pieza colocada con ajuste fino (Technic, slope, ventana, sticker).
- **Ejemplo:** `99206.dat same_piece_delta (-40.1, -57.3, 50)` indica que esta pieza (probablemente una **window frame**) se descentra 0.05 LDU en X, 0.65 en Y, 0.50 en Z — valores típicos de **window pieces** que se montan sobre studs pero tienen tolerance.

### Regla 10 — Las transiciones de pieza entre filas en X+Z suelen ir a Δx y Δz **simétricos** o **inversos**
...
- **Interpretación:** En muros rectangulares, el patrón es **pieza larga + pieza corta + pieza 1×1** para cerrar la esquina. La simetría `3009↔3010` indica **muros de fachada rectangular con ventana central** (brick 1×6 a ambos lados de la ventana, brick 1×4 entre la ventana y la esquina).
- **Ejemplo:** Trigrama `3009 → 3010 → 3010 (count=14)` describe exactamente el patrón "**muro 1×6 + dos muros 1×4 consecutivos**" — un paño de 14 studs de fachada que es 6+4+4.

---

## Notas finales sobre calidad de los datos

- **10252 Beetle** tiene **211 STEPs** (alta resolución temporal: cada paso tiene de media 2.7 piezas). Esto hace que los bigramas reflejen **orden de construcción**, no necesariamente adyacencia física. Los delta-deltas consecutivos sí son buenas señales de adyacencia.
- **10218 Pet Shop** tiene **sólo 1 STEP en el modelo maestro** (`step_count: 1, master_piece_count: 0`) — el main.ldr sólo referencia 17 sub-modelos. La construcción está detallada **dentro de cada sub-modelo**, donde sí hay muchos STEPs. Los bigramas del corpus Pet Shop que mostramos vienen de los sub-modelos (`total_pieces_in_subbuilds: 1998`).
- **Beetle = 150 piezas únicas + 81 sub-modelos** → es un modelo "complejo con muchas piezas especiales reutilizadas".
- **Pet Shop = 206 piezas únicas + 17 sub-modelos** → más "building system" clásico, menos sub-ensambles.

El contraste entre ambos corpus confirma que los patrones de encadenamiento son **genéricos del sistema LEGO** (studs cada 20 LDU, plates de 8 LDU, bricks de 24 LDU) y **no específicos de un set**. Las diferencias (4° de rotación, paneles de 4 studs vs 6 studs) reflejan la **forma del modelo final**, no la gramática de encadenamiento.
